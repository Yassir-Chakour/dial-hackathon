...
def find_impacted_recommendations(recommendations: list[dict[str, Any]], change_set: dict[str, Any]) -> tuple[Impact, ...]:
    ...
    for rec in recommendations:
        ...
        evidence_changed = set(change_set.get("evidence_ids", [])) & set(rec.get("evidence_ids", []))
        if changed & rec_records:
            result.append(Impact(rec["id"], "input_record_replaced", tuple(sorted(changed & rec_records))))
        elif evidence_changed:
            result.append(Impact(rec["id"], "evidence_superseded", (), tuple(sorted(evidence_changed))))
        elif not changed and not change_set.get("evidence_ids") and source_ids & rec_sources:
...
def mark_recommendation_stale(recommendation: Any, reason: str) -> Any:
    ...
    recommendation.superseded_at = recommendation.superseded_at or datetime.now(timezone.utc)

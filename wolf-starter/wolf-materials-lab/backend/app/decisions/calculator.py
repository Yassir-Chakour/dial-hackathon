...
from app.decisions.contracts import DecisionInputSnapshot, DecisionRecord, DecisionResult
...
def calculate_decision_facts(snapshot: DecisionInputSnapshot, records: tuple[DecisionRecord, ...] | list[DecisionRecord]) -> DecisionResult:
    ...
    amount = sum((r.baseline_value - r.value for r in rows), Decimal("0"))
    category = "recurring_saving" if amount >= 0 else "price_change"
    return _result(snapshot, payload, category, amount, rows[0].currency, rows[0].unit,
                   ("comparable_baseline",), (), True)


def _result(snapshot: DecisionInputSnapshot, payload: Any, category: str, amount: Decimal | None,
